...
class CameraWorker:
    def __init__(self, cam_cfg):
        ...
        self.detector = AnomalyDetector(self.id)
    ...
    def _run(self):
        ...
        while self.running:
            ...
            while self.running:
                ...
                is_anomaly, annotated, boxes = self.detector.analyze(frame)
                ...
                if is_anomaly:
                    self._handle_anomaly(annotated)

                time.sleep(delay)

            cap.release()

    def _handle_anomaly(self, annotated_frame):
        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        filename = f"{self.id}_{timestamp}.jpg"
        path = os.path.join(config.SNAPSHOT_DIR, filename)
        cv2.imwrite(path, annotated_frame)

        log_alert(self.id, self.name, filename)
...
def log_alert(camera_id, camera_name, snapshot_file):
    conn = get_db()
    conn.execute(
        "INSERT INTO alerts (camera_id, camera_name, snapshot_file, created_at) VALUES (?, ?, ?, ?)",
        (camera_id, camera_name, snapshot_file, datetime.now().isoformat(timespec="seconds"))
    )
    conn.commit()
    conn.close()
